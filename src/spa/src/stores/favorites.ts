...
export const useFavoritesStore = defineStore('favorites', () => {
  /** 轻量索引（启动时加载） */
  const index = ref<FavoriteIndex[]>([])
  /** 按年份缓存全量数据 */
  const yearCache = ref<Map<string, Favorite[]>>(new Map())
  /** 已加载完成的年份集合 */
  ...
  const yearGroups = computed(() => {
    const map = new Map<string, FavoriteIndex[]>()
  ...
  async function init() {
    ...
    error.value = ''
    try {
      index.value = await loadFavoritesIndex()
    } catch (e: any) {
      error.value = e?.message || '收藏索引加载失败'
      console.warn('[favoritesStore] 收藏索引加载失败，可能尚未导出', e)
    } finally {
      loading.value = false
    }
  }
  ...
  /** 按需加载某年全量数据 */
  async function loadYear(year: string | number): Promise<Favorite[]> {
    const y = String(year)
    ...
    try {
      const items = await loadFavoritesByYear(y)
      yearCache.value.set(y, items)
      loadedYears.value.add(y)
      return items
    } catch (e: any) {
      console.warn(`[favoritesStore] 加载年份 ${y} 失败`, e)
      return []
    } finally {

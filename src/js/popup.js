(function() {
    ...
    // MV3 改造：原 declarativeContent 仅在 QQ 空间页面显示 PageAction
    // MV3 中 action.default_popup 使图标始终可点击，需在 popup 内自行检测 URL
    // 当前非 QQ 空间页面时，禁用备份按钮并显示提示
    chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
        const url = tabs[0] && tabs[0].url || '';
        const isQZone = /^https:\/\/user\.qzone\.qq\.com\/\d+/i.test(url);
        if (!isQZone) {
            // 显示提示
            const $tip = $('<p style="color:#dc3545;font-weight:bold;margin-top:8px;">⚠ 请先在 QQ 空间用户主页打开本助手</p>');
            $('body > div:first').append($tip);
            // 禁用备份按钮（popup.html 中按钮 id 为 "backup"）
            $("#backup").prop('disabled', true).attr('title', '请在 QQ 空间页面使用');
            // 禁用所有模块勾选框
            $("input[name='exportType']").prop('disabled', true);
        }
    });

(function() {
    ...
    // 批量应用导出方式到所有模块
    $('#apply_export_format_all').click(function() {
        const value = $('#common_export_format').val();
        const moduleIds = ['messages','blogs','diaries','photos','videos',
                          'boards','friends','favorites','shares','visitors'];
        let applied = 0;
        let skipped = 0;
        moduleIds.forEach(id => {
            const $sel = $('#' + id + '_exportFormat');
            if ($sel.length === 0) {
                skipped++;
                return;
            }
            // 只设置该模块支持的 option，避免设置不存在的值
            if ($sel.find('option[value="' + value + '"]').length > 0) {
                $sel.val(value).change();
                applied++;
            } else {
                skipped++;
                console.info('模块 ' + id + ' 不支持 ' + value + '，已跳过');
            }
        });
        console.info('批量应用导出方式完成', { value, applied, skipped });
    });

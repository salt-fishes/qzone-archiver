...
describe('logger 文件落盘与轮转', () => {
  ...
  it('超过 maxBytes 触发轮转：main.log→.1→.2，最旧删除', () => {
    ...
    expect(fs.existsSync(path.join(dir, 'main.log.1'))).toBe(true);
    // 最旧档案不超过 maxArchives
    expect(fs.existsSync(path.join(dir, `main.log.${logger.maxArchives + 1}`))).toBe(false);
  });

  it('轮转后新 main.log 从空开始继续追加', () => {
    logger.maxBytes = 200;

...
QZoneExporters.Favorites = {
  ...
  /**
   * 导出收藏（P2-4：迁自 modules/favorites.js exportAllToFiles）
   * @param {Array} favorites 收藏列表
   */
  exportAllToFiles: async(favorites) => {
    // 获取用户配置
  ...
  /**
   * 获取单篇收藏的Markdown内容（P2-4：迁自 modules/favorites.js getMarkdown）
   * @param {object} favorite 收藏
   */
  getMarkdown: (favorite) => {
    let contents = [];
    // 获取收藏的类型

...
const Default_Config = {
    ...
    Messages: {
        ...
        Like: {
            isGet: true, //是否获取赞
            randomSeconds: {
                min: 0.3, // 点赞页间间隔（原 1~2s 过慢，点赞多的说说每页都睡 1s+ 会拖很久；0.3~0.8s 仍保留限流缓冲）
                max: 0.8
            }
        },
        Visitor: {
...
/**
 * SPA 导出资源文件清单
 * original: 扩展内资源路径（构建产物在 src/export/spa-dist/）
 * target:   备份 ZIP 内目标路径（统一放 Common/spa/ 供各模块共享引用）
 * 注意：original 指向的资源在「阶段 2」SPA 构建完成后才会存在；
 *       阶段 1 仅完成调用链与配置声明，不应真正触发 SPA 备份。
 * 维护：vite.config.ts 采用 IIFE + inlineDynamicImports，产物为单个 index.js + style.css；
 *       新增静态资源（图片、字体等）时需同步更新本清单。
 */

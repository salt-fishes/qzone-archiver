...
QZoneCollectors.Diaries = {
  ...
  /**
   * 获取所有日记的内容（迁移自 modules/diaries.js getAllContents）
  ...
  /**
   * 获取日记阅读数（迁移自 modules/diaries.js getAllReadCount）
   * @param {Array} items 日志列表
   */
  ...
  /**
   * 处理日志的图片（迁移自 modules/diaries.js handerImages）
   * @param {object} item 日志
   * @param {Array} images 图片元素列表
   */
  handerImages: async(item, images) => {
    if (!images) {
        return item;
    }
  ...
  /**
   * 处理视频信息（迁移自 modules/diaries.js handerMedias）
   * @param {object} item 日志

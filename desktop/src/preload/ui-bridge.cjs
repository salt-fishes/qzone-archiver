...
const api = {
  ...
  backup: {
    ...
    getHistory: () => invoke('backup:get-history'),
    listAlbums: () => invoke('backup:list-albums'),
    engineInject: () => invoke('backup:engine-inject'),
  },
  download: {
    ...
    clearDone: () => invoke('download:clear-done'),
  ...
  fs: {
    ...
    scanBackups: (root) => invoke('fs:scan-backups', { root }),
    saveDialog: (opts) => invoke('fs:save-dialog', opts || {}),
    writeText: (path, content) => invoke('fs:write-text', { path, content }),
  },
  zip: {

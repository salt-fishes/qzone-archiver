...
// v5.2 S2：产物目录单一常量——桌面端运行时读取的预构建查看器位置
// （engine/exporters/common.js 按 QZone.Common.SpaExportFiles 清单把产物拷进归档 Common/spa/）
const OUT_DIR = resolve(__dirname, '../src/engine/export-resources/spa-dist')
...
export default defineConfig({
  plugins: [
    ...
    fixHtmlForFileProtocol(),
    {
      ...
      apply: 'build',
      closeBundle() {
        const cssPath = resolve(OUT_DIR, 'assets/style.css')
